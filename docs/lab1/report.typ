...
В ходе выполнения лабораторной работы я доработала систему, добавив в систему возможность массового добавления объектов при помощи импорта файла (json).
Также добавила в систему интерфейс для отображения истории импорта, реализовал дополнительные ограничения на уровне бизнес-логики, такие как:
- Создание групп с одинкаовым именем и формой обучения запрещено
- Создание админов с одинаковым именем и адресом (локацией) также запрещено
А ещё изучила уровни изоляции транзакций, реализовала создание сценария с использованием Apache JMeter.