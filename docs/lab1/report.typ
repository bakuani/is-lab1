...
public class Coordinates {
    private double x;
    private double y; //Максимальное значение поля: 697
}
public class Person {
    private String name; //Поле не может быть null, Строка не может быть пустой
...
- Вернуть количество объектов, значение поля semesterEnum которых меньше заданного.
- Вернуть массив объектов, значение поля groupAdmin которых меньше заданного.
- Вернуть массив уникальных значений поля shouldBeExpelled по всем объектам.
...
#figure(
  image("is-lab1.drawio.png", width: 100%)
)

#figure(
  image("package.drawio.png", width: 100%)
)

#pagebreak()

= Ссылка на репозиторий с исходным кодом
GitHub: https://github.com/bakuani/is-lab1
...
- Повторила и применила на практике основные методы HTTP-запросов\
- Настроила хранение объектов в базе данных с использованием ORM\
- Реализовала взаимодействие с объектами через HTTP-запросы